...
export default function Certifications({ data, onSave, isSaving }) {
  ...
  return (
    <section className="bg-slate-900/50 backdrop-blur-md p-8 rounded-3xl shadow-2xl border border-white/10 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-600/5 rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/2" />
      <h2 className="text-3xl font-bold mb-6 text-white relative z-10">Certifications</h2>

      <div className="space-y-6">
        {rows.map((r) => (
          <div
            key={r.id}
            className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end p-6 bg-slate-800/40 border border-white/5 rounded-2xl shadow-sm hover:border-white/10 transition group relative z-10"
          >
            <input
              className="md:col-span-2 p-3.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all placeholder:text-slate-500"
